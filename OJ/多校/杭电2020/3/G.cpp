...
long long calc(point now,point v,point A,point B,double L,double dis){
    if(parallel(A,B,point(0,0),v)) return 0;
    double pre=dis_point_to_line(now,A,B);
  //  cout<<pre<<" "<<A.x<<" "<<A.y<<" "<<B.x<<" "<<B.y<<endl;
    if((v^(B-A))<0){
        pre=sqrt(3.0)/2*L-pre;
    }
    dis=(long double)dis*fabs(v^(B-A))/length(B-A)/length(v);
   // cout<<dis<<" "<<pre<<endl;
    if(dis>=pre) dis-=pre;
    else return 0;
    return (long long)(dis/(sqrt(3.0)/2*L)+eps)+1;
}
int main(){
    int t=read();
    while(t--){
        double L=read(),x=read(),y=read(),vx=read(),vy=read();
        int K=read();
        point A(-L/2,0),B(L/2,0),C(0,sqrt(3.0)/2*L);
        point ball(x,y),ve(vx,vy);
        double l=0,r=1e10+0.5,ans=0;
        for(int i=1;i<=300;i++){
            double mid=(l+r)/2.0;
            if(calc(ball,ve,B,C,L,mid)+calc(ball,ve,C,A,L,mid)+calc(ball,ve,A,B,L,mid)>=K){
                r=mid;ans=mid;
            }
            else l=mid;
        }
