...
int main(){
    ...
    while(n--){
        ...
        if(f<h) swap(f,h);
        int ok=0;
        if(intersect_segment(point(a,b),point(c,d),point(e,f),point(g,f))) ok=1;
        if(intersect_segment(point(a,b),point(c,d),point(e,h),point(g,h))) ok=1;
        if(intersect_segment(point(a,b),point(c,d),point(e,f),point(e,h))) ok=1;
        if(intersect_segment(point(a,b),point(c,d),point(g,f),point(g,h))) ok=1;
        if(a>=e&&a<=g&&b>=h&&b<=f&&c>=e&&c<=g&&d>=h&&d<=f) ok=1;
        if(ok) printf("T\n");
        else printf("F\n");
    }
    return 0;
