...
int K; 
void build(int n){
    if(aa.count(n)) return;
    if(n==0){
        aa[n]=Poly{1};aa[n].resize(K);return;
    }
    else if(n==1){
        aa[n]=Poly{1,1};aa[n].resize(K);return;
    }
    int mid1=n/2,mid2=(n+1)/2;
    build(mid1);build(mid2);build(mid1-1);build(mid2-1);
    Poly tmp2=aa[mid1-1]*aa[mid2-1];
    Poly tmp3=Poly{0};tmp3.insert(tmp3.end(),tmp2.begin(),tmp2.end());
    aa[n]=aa[mid1]*aa[mid2]+tmp3;aa[n].resize(K);
} 
int main(){
	init_inv();
    int n=read(),m=read();K=m+1;
    build(n);
    for(int i=1;i<=m;i++) cout<<aa[n][i]<<" ";
	return 0;
}