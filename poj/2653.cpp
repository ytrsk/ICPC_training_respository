...
int main(){
    ...
    while((n=read())&&n){
        ...
        aa.clear();
        for(int i=1;i<=n;i++){
            map<int,int>::iterator nex;
            for(map<int,int>::iterator it=aa.begin();it!=aa.end();it=nex){
                nex=it;nex++;
                if(intersect_segment(seg[it->first].a,seg[it->first].b,seg[i].a,seg[i].b)){
                   aa.erase(it);
                }
            }
            aa[i]=1;
